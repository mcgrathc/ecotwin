...
daily_soil_water_deficit, hourly_soil_water_deficit = process_data(data_directory='Y:\\data_ZentraCloud\\', metadata_file='Y:\\metadata.csv')
...
def plot_soil_water(soil_data, plot_name):
    ...
    for i, depth in enumerate(unique_depths):
        ...
        for year in subset['year'].unique():
            ...
            axs[i].plot(subset_year['datetime'], subset_year['diff_from_irr_trt'], color=color, label=f'Year {year} Depth {depth}cm')
